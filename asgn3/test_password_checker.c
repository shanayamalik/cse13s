...
void main() {
    ...
    printf("Congrats! The first test case is now passing. You should remove the assert statements that you added to "
           "password_checker.c because these correspond to the first test case and will not necessarily work for the "
    ...
    const char *test8_first = "Barack";
    const char *test8_last = "Obama";
    const char *test8_pwd = "dmVyeSBjbGV2ZXIh";
    bool test8 = check_password(test8_first, test8_last, test8_pwd);
    assert(test8);

    const char *test9_first = "Leslie";
    const char *test9_last = "Lamport";
    const char *test9_pwd = "YWI9YmE9Pw";
    bool test9 = check_password(test9_first, test9_last, test9_pwd);
    assert(test9);

    const char *test10_first = "Edsger";
    const char *test10_last = "Dijkstra";
    const char *test10_pwd = "Aa0aAaaaaa";
    bool test10 = check_password(test10_first, test10_last, test10_pwd);
    assert(test10);
